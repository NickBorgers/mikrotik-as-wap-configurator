    ...
    for (const ssidConfig of ssids) {
      ...
      for (const band of bands) {
        ...
        try {
          ...
          // Build steering parameters for 802.11k/v
          let steeringParams = '';
          if (useRRM) steeringParams += ` steering.rrm=yes`;
          if (useWNM) steeringParams += ` steering.wnm=yes steering.transition-threshold=${transitionThreshold}`;
