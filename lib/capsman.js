...
/**
 * Ensure WiFi interfaces are added to bridge on CAP device
 * This is necessary for IGMP snooping to work correctly - virtual WiFi
 * interfaces may not be automatically added as bridge ports on CAP devices.
 *
 * @param {Object} capConfig - CAP device configuration
 * @param {string} capConfig.host - CAP hostname/IP
 * @param {string} capConfig.username - SSH username
 ...
 * @returns {boolean} - Success status
 */
async function ensureCapWifiInBridge(capConfig) {
  const mt = new MikroTikSSH(
    capConfig.host || '192.168.88.1',
    capConfig.username || 'admin',
    capConfig.password || 'admin'
  ...
  try {
    ...
    const identity = capConfig.identity || capConfig.host.split('.')[0];
    console.log(`\n--- Ensuring WiFi Bridge Ports: ${identity} ---`);
    ...
    // Ensure all WiFi interfaces are in bridge
    await ensureWifiInterfacesInBridge(mt, wifiPath);

    await mt.close();
    return true;
  } catch (error) {
    console.error(`\n✗ Bridge port check error for ${capConfig.host}: ${error.message}`);
    await mt.close();
    return false;  // Non-fatal - continue with other CAPs
  }
}
