...
async function configureAccessLists(controllerConfig, lockedDevices, deploymentSsids, devices) {
  ...
  try {
    ...
    // Remove ALL existing WAP locking rules (rules with "lock to" or "locked to" in comment)
    // This ensures rules for devices removed from config are cleaned up
    console.log('=== Cleaning Up All WAP Locking Rules ===');
    try {
      // Remove rules that match locking comment patterns
      // Comment patterns: "hostname - lock to ap" or "hostname - reject (locked to ap)"
      await mt.exec(`${wifiPath}/access-list remove [find comment~"lock to" or comment~"locked to"]`);
      console.log('  ✓ Removed all existing WAP locking rules');
    } catch (e) {
      if (!e.message.includes('no such item')) {
        console.log(`  ⚠️  Could not clean up locking rules: ${e.message}`);
      } else {
        console.log('  No existing WAP locking rules found');
      }
    }

    // If no locked devices to configure, we're done after cleanup
    if (!hasLockedDevices) {
